// Phase 2, perform replacements for non templates

use std::{borrow::Cow, collections::HashSet, mem};

use crate::{
    html::{self, Element, Replacer, RootElement},
    preprocess::EnvValue,
};
    ...
    F: FnMut(&str) -> Option<EnvValue<'a>>,
{
    for child in tree.iter_mut() {
        if let RootElement::Element(elem) = child {
            process_element(elem, &mut env_fetcher)?;
        }
    }
    Ok(())
}
        ...
        match attribute {
            html::Attribute::Replacer(replacer) => {
                match process_replacer(replacer, &mut env_fetcher)? {
                    EnvValue::Elements(_) => {
                        return Err("Cannot place Elements in attributes list".to_string());
                    }
                    EnvValue::String(_) => {
                        return Err("Cannot place String in attributes list".to_string());
                    }
                    EnvValue::Attributes(attrs) => {
                        for attribute in attrs {
                            match attribute {
                                html::Attribute::Comment(_, _) => (),
                                html::Attribute::Replacer(_) => {
                                    return Err(
                                        "Cannot have replacer replacing replacer".to_string()
                                    );
                                }
    ...
    for child in old_content {
        match child {
            html::ElementContent::Element(mut child) => {
                ...
                // to nest badly, trying to find requirements
                process_element(&mut child, &mut env_fetcher as &mut dyn FnMut(&str) -> Option<EnvValue<'a>>)?;
                ...
                match process_replacer(&replacer, &mut env_fetcher)? {
                    EnvValue::Elements(elements) => {
                        for replaced in elements {
                            element
                                .content
                                .push(html::ElementContent::Element(replaced.clone()));
                        }
                    }

                    EnvValue::Attributes(_) => {
                        return Err(
                            "Attributes replacer cannot be inserted to text portion of element"
                                .to_string(),
                        );
                    }

                    EnvValue::String(replaced) => {
                        element.content.push(html::ElementContent::Text(
                            html::Span {
                                start: html::Location {
                                    line: 0,
                                    column: 0,
...
fn process_replacer<'a, F>(
    replacer: &Replacer<'_>,
    mut env_fetcher: F,
) -> Result<EnvValue<'a>, String>
where
    F: FnMut(&str) -> Option<EnvValue<'a>>,
{
    match replacer {
        html::Replacer::Complex(_, _) => {
            return Err(
                "Complex syntax like ${...} is reserved for something, do not use".to_string(),
            );
        }
